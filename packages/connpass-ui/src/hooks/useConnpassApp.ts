...
import {
  extractEventDetailData,
  extractEventSearchData,
  extractPrefectureListData,
  extractScheduleData,
...
import {
  ...
  createConnpassToolClient,
} from "./connpassToolClient";
...
function inferDatePreset(
  from: string,
  to: string,
): SearchFormValues["datePreset"] {
  const entries: Array<Exclude<SearchFormValues["datePreset"], "custom">> = [
    "today",
    "tomorrow",
    "this-week",
    "this-month",
...
const defaultSearchFormValues: SearchFormValues =
...
function normalizeSearchFormValues(
  ...
  return {
    ...
    minParticipants:
      typeof params.minAccepted === "number" ? String(params.minAccepted) : "",
    minCapacity:
      typeof params.minCapacity === "number" ? String(params.minCapacity) : "",
    sort:
      params.sort === "participant-count-desc" ||
      params.sort === "title-asc" ||
      params.sort === "start-date-asc"
        ? params.sort
        : "start-date-asc",
    showAdvanced:
...
export function useConnpassApp() {
  ...
  const toolClientRef = useRef<ReturnType<
    typeof createConnpassToolClient
  > | null>(null);
      ...
      try {
        ...
        app.ontoolinput = (input) => {
          ...
          const isEventTool =
            ...
            toolName === "browse_events" ||
            toolName === "_search_events" ||
            toolName === "";
          if (isEventTool) {
            const formValues = normalizeSearchFormValues(
          ...
          const isScheduleTool =
            toolName === "search_schedule" || toolName === "_search_schedule";
          if (isScheduleTool) {
            const args = (input?.arguments ?? {}) as Record<string, unknown>;
            const nickname =
              typeof args.nickname === "string" ? args.nickname : "";
            setState((prev) => ({
              ...prev,
            ...
            if (data) {
              setState((prev) => ({
                ...
                searchFormValues: {
                  ...
                  pageSize:
                    data.returned > 0
                      ? snapPageSize(data.returned)
                      : prev.searchFormValues.pageSize,
                },
              }));
              return;
            }
  ...
  const searchEvents = useCallback(async (params: SearchEventsParams) => {
    ...
    setState((prev) => ({
      ...
      searchFormValues: {
        ...
        datePreset:
          (normalizedParams.datePreset as SearchFormValues["datePreset"]) ??
          prev.searchFormValues.datePreset,
        prefecture:
          typeof normalizedParams.prefectures === "string"
            ? normalizedParams.prefectures
            : Array.isArray(normalizedParams.prefectures) &&
                typeof normalizedParams.prefectures[0] === "string"
              ? normalizedParams.prefectures[0]
              : "",
        company: normalizedParams.companyQuery ?? "",
        minParticipants:
          typeof normalizedParams.minAccepted === "number"
            ? String(normalizedParams.minAccepted)
            : "",
        minCapacity:
          typeof normalizedParams.minCapacity === "number"
            ? String(normalizedParams.minCapacity)
            : "",
    ...
    try {
      ...
      setState((prev) => ({
        ...
        error: result.isError
          ? (extractToolErrorMessage(result) ?? "Search failed")
          : null,
      }));
    } catch (e) {
      setState((prev) => ({
  ...
  const selectEvent = useCallback(async (event: FormattedEvent) => {
    ...
    try {
      ...
      setState((prev) => ({
        ...
        detailLoading: false,
        error: result.isError
          ? (extractToolErrorMessage(result) ?? "Failed to load event detail")
          : null,
      }));
    } catch (e) {
      setState((prev) => ({
