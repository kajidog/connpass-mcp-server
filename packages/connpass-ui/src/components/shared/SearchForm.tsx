...
import { type DatePresetKey, DATE_PRESET_OPTIONS, createDatePreset } from '../../utils/datePreset'
...
export function SearchForm({ values, prefectures, onSearch, loading }: SearchFormProps) {
  const [formValues, setFormValues] = useState<SearchFormValues>(() => normalizeValues(values))
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSearch({
      ...
      minAccepted: formValues.minParticipants ? Number(formValues.minParticipants) : undefined,
      minCapacity: formValues.minCapacity ? Number(formValues.minCapacity) : undefined,
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 p-3" style={{ background: 'var(--ui-surface)', borderRadius: '8px' }}>
      {/* OR検索キーワード */}
      <div className="flex flex-col gap-1">
        <label htmlFor="connpass-any-query" className="text-xs font-medium" style={{ color: 'var(--ui-text)' }}>
          キーワード
        </label>
        <span className="text-[11px]" style={{ color: 'var(--ui-text-secondary)' }}>
        ...
        <input
          ...
          onChange={(e) => setFormValues((prev) => ({ ...prev, anyQuery: e.target.value }))}
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            onClick={() => setFormValues((prev) => ({ ...prev, datePreset: 'custom' }))}
            className="px-3 py-1.5 rounded-full text-xs"
            style={{
        ...
        {isCustomDate && (
          <div className="grid grid-cols-2 gap-2 mt-1">
            <div className="flex flex-col gap-0.5">
              <label htmlFor="connpass-from" className="text-[11px]" style={{ color: 'var(--ui-text-secondary)' }}>
                開始日
              </label>
              <input
                id="connpass-from"
                ...
                onChange={(e) => setFormValues((prev) => ({ ...prev, from: e.target.value }))}
                className="px-2 py-1.5 rounded-md text-xs"
                style={{ background: 'var(--ui-bg)', border: '1px solid var(--ui-border)', color: 'var(--ui-text)' }}
              />
            </div>
            <div className="flex flex-col gap-0.5">
              <label htmlFor="connpass-to" className="text-[11px]" style={{ color: 'var(--ui-text-secondary)' }}>
                終了日
              </label>
              <input
                id="connpass-to"
      ...
      <div className="grid grid-cols-[1fr_auto_auto] gap-2 items-end">
        <button
          type="button"
          onClick={() => setFormValues((prev) => ({ ...prev, showAdvanced: !prev.showAdvanced }))}
        ...
        <div className="flex flex-col gap-0.5">
          <label htmlFor="connpass-page-size" className="text-[11px] text-right" style={{ color: 'var(--ui-text-secondary)' }}>
            検索件数
          </label>
          <select
            id="connpass-page-size"
            value={String(formValues.pageSize)}
            onChange={(e) => setFormValues((prev) => ({ ...prev, pageSize: Number(e.target.value) }))}
        ...
        <div className="flex flex-col gap-0.5">
          <label htmlFor="connpass-sort" className="text-[11px] text-right" style={{ color: 'var(--ui-text-secondary)' }}>
            ソート
          </label>
          <select
            id="connpass-sort"
            value={formValues.sort}
            onChange={(e) => setFormValues((prev) => ({ ...prev, sort: e.target.value as SearchFormValues['sort'] }))}
            className="px-2 py-2 rounded-md text-xs"
            style={{ background: 'var(--ui-bg)', border: '1px solid var(--ui-border)', color: 'var(--ui-text)' }}
          >
      ...
      {formValues.showAdvanced && (
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="col-span-2 flex flex-col gap-1">
            <label htmlFor="connpass-prefecture" className="text-xs font-medium" style={{ color: 'var(--ui-text)' }}>
              都道府県
            </label>
            <span className="text-[11px]" style={{ color: 'var(--ui-text-secondary)' }}>
              開催地域で絞り込みます
            ...
            <select
              ...
              onChange={(e) => setFormValues((prev) => ({ ...prev, prefecture: e.target.value }))}
          ...
          <div className="col-span-2 flex flex-col gap-1">
            <label htmlFor="connpass-company" className="text-xs font-medium" style={{ color: 'var(--ui-text)' }}>
              会社名・主催名
            </label>
            <span className="text-[11px]" style={{ color: 'var(--ui-text-secondary)' }}>
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="connpass-min-participants" className="text-xs font-medium" style={{ color: 'var(--ui-text)' }}>
              参加人数以上
            </label>
            <input
              ...
              onChange={(e) => setFormValues((prev) => ({ ...prev, minParticipants: e.target.value }))}
              className="px-3 py-2 rounded-md text-xs"
              style={{ background: 'var(--ui-bg)', border: '1px solid var(--ui-border)', color: 'var(--ui-text)' }}
            />
          </div>
          <div className="flex flex-col gap-1">
            <label htmlFor="connpass-min-capacity" className="text-xs font-medium" style={{ color: 'var(--ui-text)' }}>
              募集人数以上
            </label>
            <input
              ...
              onChange={(e) => setFormValues((prev) => ({ ...prev, minCapacity: e.target.value }))}
              className="px-3 py-2 rounded-md text-xs"
              style={{ background: 'var(--ui-bg)', border: '1px solid var(--ui-border)', color: 'var(--ui-text)' }}
            />
          </div>
