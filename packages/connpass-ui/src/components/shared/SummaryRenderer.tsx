...
function ScheduleTable({
  ...
  const gridTemplateColumns = hasSpeakerColumn
    ...
    >
      ...
      >
        ...
        <div className="px-3 py-2">
          {hasSpeakerColumn ? (headers[2] ?? "内容") : headers[1]}
        </div>
      </div>
...
function GenericTable({
  block,
}: { block: Extract<SummaryBlock, { type: "table" }> }) {
  ...
  switch (block.type) {
    case "heading":
      return (
        <h3
          key={index}
          className="text-base font-bold mt-5 first:mt-0"
          style={{ color: "var(--ui-text)" }}
        >
          {block.text}
        </h3>
        ...
        >
          {block.rows.map((row, rowIndex) => (
            <div
              ...
              style={{
                borderTop:
                  rowIndex === 0 ? "none" : "1px solid var(--ui-border)",
              }}
            >
              <div
                className="px-3 py-2 font-medium"
    ...
    case "schedule":
      return (
        <ScheduleTable key={index} block={block} onOpenLink={onOpenLink} />
      );
    default:
      return null;
  }
