...
export function buildSummaryBlocks(summary: string | undefined | null): SummaryBlock[] {
    ...
    if (isScheduleLine(current)) {
      ...
      while (index < segments.length && isScheduleLine(segments[index])) {
        ...
        while (index < segments.length) {
          const peek = segments[index]
          const normalizedPeek = normalizeHeadingText(peek)
          if (SUMMARY_SECTION_HEADINGS.has(normalizedPeek) || SUMMARY_SKIP_HEADINGS.has(normalizedPeek) || peek.startsWith('## ')) {
            break
          }
      ...
      if (entries.length > 0) {
        blocks.push({
          type: 'schedule',
          entries,
          headers: pendingScheduleHeaders.length > 0 ? pendingScheduleHeaders.slice(0, 3) : undefined,
        })
      }
      pendingScheduleHeaders = []
      continue
