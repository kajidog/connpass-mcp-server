import type { DetailFact, EventSearchResult, FormattedEvent, PrefectureOption, ScheduleResult } from './types'
 ...
 */
export function extractEventSearchData(result: unknown): EventSearchResult | null {
...
export function extractPrefectureListData(result: unknown): PrefectureOption[] | null {
  const data = unwrapToolPayload(result)
  if (!data) return null
...
export function buildDetailFacts(event: FormattedEvent): DetailFact[] {
  ...
  if (event.owner.displayName || event.owner.nickname) {
    facts.push({
      label: '主催',
      value: [event.owner.displayName, event.owner.nickname ? `@${event.owner.nickname}` : ''].filter(Boolean).join(' '),
    })
  }
  ...
  if (event.location?.place || event.location?.address) {
    facts.push({
      label: '会場',
      value: [event.location.place, event.location.address].filter(Boolean).join(' / '),
    })
  }
