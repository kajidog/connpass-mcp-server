...
const ALL_PREFECTURES = Object.entries(PREFECTURES).map(([code, name]) => ({ code, name }));
...
export function isKnownPrefecture(value: string): boolean {
  ...
  return PREFECTURE_NAME_SET.has(normalized) || Object.prototype.hasOwnProperty.call(PREFECTURES, normalized.toLowerCase());
}

export function normalizePrefecture(value: string): string | undefined {
  const normalized = value.trim();
  if (!normalized) return undefined;
  if (Object.prototype.hasOwnProperty.call(PREFECTURES, normalized.toLowerCase())) {
...
export function normalizePrefectureList(values?: string | string[]): string[] | undefined {
