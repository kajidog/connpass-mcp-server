...
export async function startHttpServer(options: LaunchOptions): Promise<void> {
  ...
  try {
    ...
    if (isBun()) {
      ...
      console.error(
        `${serverName} HTTP server running at http://${bunServer.hostname}:${bunServer.port}/mcp`,
      );
      console.error(
        `Health check: http://${bunServer.hostname}:${bunServer.port}/health`,
      );
    } else {
      ...
      serve(
        ...
        (info: ServerInfo) => {
          console.error(
            `${serverName} HTTP server running at http://${info.address}:${info.port}/mcp`,
          );
          console.error(
