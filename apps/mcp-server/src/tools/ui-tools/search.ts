...
const UIEventSearchInputSchema = z.object({
  ...
  prefectures: z.union([z.string().min(1), z.array(z.string().min(1))]).optional(),
  ...
  sort: z.enum(['start-date-asc', 'participant-count-desc', 'title-asc']).optional(),
})

function includesNormalized(targets: Array<string | undefined>, query?: string): boolean {
...
function applyLocalFilters(events: Event[], params: z.infer<typeof UIEventSearchInputSchema>) {
  return events.filter((event) => {
    if (!includesNormalized([event.ownerDisplayName, event.groupTitle], params.companyQuery)) {
      return false
    }
...
function sortEvents(events: Event[], sort: z.infer<typeof UIEventSearchInputSchema>['sort']) {
  const copied = [...events]
  copied.sort((a, b) => {
...
export function registerUISearchTool(deps: ToolDeps): void {
  ...
  registerAppToolIfEnabled(server, '_search_events', {
    ...
    if (!useLocalFiltering) {
      const formatted = formatEventsResponse(response, UI_LIST_FORMAT_OPTIONS)
      return {
    ...
    const filtered = sortEvents(applyLocalFilters(response.events, params), params.sort)
    ...
    const formatted = {
      ...
      events: paged.map((event) => formatEvent(event, UI_LIST_FORMAT_OPTIONS)),
    }
    return {
      content: [{ type: 'text' as const, text: JSON.stringify(formatted) }],
      structuredContent: {
