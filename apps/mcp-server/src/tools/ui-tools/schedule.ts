...
export function registerUIScheduleTool(deps: ToolDeps): void {
  ...
  registerAppToolIfEnabled(server, '_search_schedule', {
    ...
    if (!userNickname) {
      ...
      if (!userNickname) {
        ...
        if (userResponse.users.length === 0) throw new Error(`User with ID ${resolvedUserId} not found.`)
    ...
    const sections = sortedDates.map((date) => ({
      date,
      events: formatEventList(eventsByDate.get(date)!, SCHEDULE_FORMAT_OPTIONS),
    }))
