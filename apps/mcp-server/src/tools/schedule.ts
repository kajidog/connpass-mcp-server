...
const ScheduleInputSchema = z.object({
  ...
  fromDate: z.string().min(1).describe('Start date (YYYY-MM-DD). Defaults to today.').optional(),
  toDate: z.string().min(1).describe('End date (YYYY-MM-DD). Defaults to +7 days.').optional(),
  maxEvents: z.number().int().min(1).max(100).describe('Maximum events to check (default 30)').optional(),
  includeDetails: z.boolean().describe('Include event description (up to 200 chars)').default(false).optional(),
})
...
export function registerScheduleTools(deps: ToolDeps): void {
  ...
  registerAppToolIfEnabled(server, 'search_schedule', {
    ...
    let resolvedUserId: number | undefined = parsed.userId ?? config.defaultUserId
    ...
    if (!resolvedUserId) {
      throw new Error('User ID or nickname is required. Pass userId, nickname, or set CONNPASS_DEFAULT_USER_ID.')
    }
    ...
    if (!userNickname) {
      ...
      userNickname = userResponse.users.find((u) => u.id === resolvedUserId)?.nickname
      if (!userNickname) throw new Error(`User with ID ${resolvedUserId} not found.`)
    }
    ...
    const sections = sortedDates.map((date) => ({
      date,
      events: formatEventList(eventsByDate.get(date)!, parsed.includeDetails ? FORMAT_PRESETS.detailed : FORMAT_PRESETS.default),
    }))
    ...
    return {
      content: [{ type: 'text' as const, text: summarizeEventsResponse({
        returned: sections.reduce((count, section) => count + section.events.length, 0),
        available: sections.reduce((count, section) => count + section.events.length, 0),
        start: 1,
