...
import { GROUP_SORT_KEYS, GROUP_SORT_MAP, applyPagination } from './utils/shared.js'

const GroupSearchInputSchema = z.object({
  query: z.string().min(1).describe('Keywords matching group title or description').optional(),
  groupIds: z.array(z.number()).describe('Limit to specific group IDs').optional(),
  country: z.string().min(1).describe("ISO country code, e.g. 'JP'").optional(),
  prefecture: z.string().min(1).describe('Prefecture name to filter by').optional(),
  page: z.number().int().min(1).describe('1-based page number').optional(),
  pageSize: z.number().int().min(1).max(100).describe('Groups per page (default 20)').optional(),
  sort: z.enum(GROUP_SORT_KEYS).describe('Ranking by activity, members, or recency').optional(),
})
...
export function registerGroupTools(deps: ToolDeps): void {
  ...
  registerAppToolIfEnabled(server, 'search_groups', {
    title: 'Search Groups',
