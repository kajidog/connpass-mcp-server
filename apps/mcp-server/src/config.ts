...
export function getConfig(argv?: string[], env?: NodeJS.ProcessEnv): ServerConfig {
  const cliConfig = parseCliFromDefs(allConfigDefs, argv ?? process.argv.slice(2)) as Partial<ServerConfig>
  const envConfig = parseEnvFromDefs(allConfigDefs, env ?? process.env) as Partial<ServerConfig>
